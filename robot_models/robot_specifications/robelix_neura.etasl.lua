...
local frames = robot_worldmodel:getExpressions(VL,ctx,{
                                                            ...
                                                            tool0_cables_debug_root = {'tool0_cables','maira7M_link6'}
                                                        })
...
mav_vel_mav = constant(0.1)
Constraint{
    context = ctx,
    name    = "mav_linear_velocity_limit",
    expr    = coord_x(origin(task_frame_inst_mobile_base)),
    target_upper = mav_vel_mav*time,
    target_lower = constant(-1)*mav_vel_mav*time,
    K       = 0,
    weight  = 1,
    priority= 0
};
