...
freq = 0.05
amplitude = constant(0.2)
traj_mav = amplitude*cos(constant(2*3.1416*freq)*time)

sine_wave_vector = make_constant(mav_base_link)*vector(traj_mav, 0, 0)

--TODO: fix constranints frames and transformations!!!!
Constraint{
    context = ctx,
    name    = "mav_x_velocity_constraint",
    expr    = coord_x(origin(task_frame_inst_mobile_base)),
    target  = traj_mav, -- Zero velocity constraint
    K       = 2,
    weight  = 7*variable_weight,
    priority= 2
};
